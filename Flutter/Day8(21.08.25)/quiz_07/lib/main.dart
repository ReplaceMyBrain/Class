...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Widget build(BuildContext context) {
    return CupertinoApp(
        ...
        home: CupertinoPageScaffold(
            backgroundColor:
                currentAlarmDateTime == _chosenItem(chosenDateTime!)
                    ? int.parse(sec) % 2 == 0
                        ? Colors.redAccent
                        : Colors.blueAccent
                    : Colors.white,
  ...
  void _addItem() {
    ...
    setState(() {
      ...
      currentAlarmDateTime =
          "${now.year}-${zero.monthZero(now.month)}-${zero.dayZero(now.day)} ${zero.weekday(now.weekday)} ${zero.hourZero(now.hour)}:${zero.minuteZero(now.minute)}:00";
      sec = "${now.second}";
    });
  }
